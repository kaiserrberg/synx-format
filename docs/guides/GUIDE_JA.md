...
> **🔗 [ロゴを表示 →](https://media.aperturesyndicate.com/asother/as/branding/png/aperturesyndicate.png)**

<h1 align="center">SYNX v3.6 — 完全ガイド</h1>
...
パッケージ: [nuget.org/packages/APERTURESyndicate.Synx](https://www.nuget.org/packages/APERTURESyndicate.Synx)。nuget.org では ID `Synx.Core` は他者が使用中です。公開前は [`parsers/dotnet/README.md`](../../parsers/dotnet/README.md) を参照。
...
SYNX v3.6は**24のマーカー**を提供します。各マーカーは `:マーカー` 構文でキーに付加される関数です。
...
> NuGet IDは `APERTURESyndicate.Synx` です（`Synx.Core` ではありません — その名前は既に使用されています）。[nuget.org/packages/APERTURESyndicate.Synx](https://nuget.org/packages/APERTURESyndicate.Synx) を参照してください。

これは**マネージド .NET 8 実装**です — ネイティブDLLは不要です。パーサーは純粋なC#で、適合テストスイートを通じてRustリファレンスと整合しています。

**SynxOptions:**

| プロパティ | 型 | 効果 |
|----------|------|--------|
| `Env` | `Dictionary<string, string>` | `:env` マーカー用の疑似環境を注入 |
| `Region` | `string` | `:geo` の値 |
| `Lang` | `string` | `:i18n` の言語 |
| `BasePath` | `string` | `:include` のベースディレクトリ |
| `MaxIncludeDepth` | `int` | インクルードのネスト上限 |
...
キーをアルファベット順にソートし、2スペースインデントに正規化し、コメントを除去します。`synx format` CLIと同じ出力です。
...
1. モノレポから `synx-c` をビルド: `cargo build --release -p synx-c`
2. `bindings/c-header/include/synx.h` と `bindings/cpp/include/synx/synx.hpp` をインクルードパスにコピー
3. `libsynx_c` (`.so` / `.dylib` / `.dll`) にリンク
...
**API (ヘッダーオンリー、`synx/synx.hpp`):**

すべての関数は `std::optional<std::string>` を返します — エラー時は `nullopt`。`compile` は `std::optional<std::vector<unsigned char>>` を返します。
...
> **メモリ:** C++ヘッダーはすべてのメモリを自動的に管理します。`synx_free` を手動で呼び出す必要はありません — optionalラッパーがデストラクタで処理します。
...
バインディングはcgoを使用し、`libsynx_c` にリンクします。

```bash
go get github.com/APERTURESyndicate/synx-format/go
```

> **cgo必須。** GoバインディングはcgoでSynx-core共有ライブラリを使用します。プラットフォーム固有のビルド手順はモジュールREADMEを参照してください。
...
SynxEngine経由のSwift Package Managerバインディング（synx-coreへのFFI）。
...
synx-core共有ライブラリ経由のJNAバインディング。任意のJVM言語で動作します。
...
WASMバインディングはnpmパッケージ `@aperturesyndicate/synx-format` の基盤です。wasm-bindgenを使用してsynx-coreをWASMにコンパイルし、JavaScript/TypeScriptグルーコードを提供します。
...
WASMビルドはCloudflare Workers、Deno Deploy、その他のWASM対応エッジランタイムと互換性があります。npmパッケージを直接使用してください — WASMバイナリをアセットとして同梱しています。

---

### Mojo

CPythonインターオプバインディング。内部的にPython `synx_native` 拡張機能を使用します。
...
または拡張機能パネルで**SYNX**を検索してください。

**機能：**

- `.synx`ファイルのシンタックスハイライト
- リアルタイム診断（タブ、不揃いのインデント、重複キー、不明なマーカー）
- マーカー、制約、ディレクティブの自動補完
- ドキュメントシンボルアウトライン
- 保存時フォーマット
- マーカーのホバードキュメント
- `:watch`によるライブリロード
...
サーバーは標準LSPプロトコルを使用して**stdio**で通信します。引数なしで`synx-lsp`として実行してください。

| 機能 | 説明 |
|------------|-------------|
| 診断 | タブ、不揃いのインデント、重複キー、不明なマーカー/制約 |
| 自動補完 | マーカー（`:env`、`:calc`、…）、制約、ディレクティブ |
| ドキュメントシンボル | ネスト付きの完全なドキュメントアウトライン |
...
**Visual Studio 2022+:** `integrations/visualstudio/`からVSIXをExtensions → Manage Extensionsでインストールしてください。

### MCPサーバー

`synx-mcp`サーバーは、MCP互換クライアント（Claude Desktop、Claude Codeなど）にSYNX操作をMCPツールとして公開します。

**利用可能なツール：**

| ツール | 説明 |
|------|-------------|
| `validate` | `.synx`ファイルの構文と制約を検証 |
| `parse` | SYNX文字列またはファイルをJSONにパース |
| `format` | SYNXドキュメントを正規フォーマットに整形 |
| `synx_read_path` | ファイル読み取り（`SYNX_MCP_ROOT`で制限） |
| `synx_write_path` | アトミック書き込み（temp + rename） |
| `synx_apply_patch` | ファイル内の部分文字列を置換 |
...
複数ルート：`"SYNX_MCP_ROOTS": "path1,path2"`。ファイルサイズ上限：10 MB。

---

## バイナリフォーマット (.synxb)

SYNXはバイナリフォーマット（`.synxb`）にコンパイルできます — 高速パースとコンパクトな保存のために。バイナリフォーマットはテキストSYNXと同じデータモデルをエンコードしますが、UTF-8テキストではなくレングスプレフィックスバイナリエンコーディングを使用します。
...
- **高速パース** — トークン化やインデントカウントが不要
- **小さいファイル** — キーのインターニングとコンパクトな整数エンコーディング
- **人間が編集不可** — 人間が編集する設定ファイルにはテキストSYNXを使用
- **ラウンドトリップセーフ** — compile → decompileは同一データを生成（同一テキストではない）

---

## 構造的Diff

2つのSYNXドキュメントを比較し、型付きの変更リストを取得：追加、削除、変更、それぞれドット区切りのキーパス付き。
...
`!active` SYNXドキュメントの制約からDraft 2020-12 JSON Schemaを生成します。
...
生成または外部のJSON Schemaに対してJSONデータを検証します。

```bash
# JSONファイルをJSON schemaに対して検証
synx json-validate data.json schema.json

# 自己検証：!activeドキュメントからschemaを生成して検証
...
すべての公式バインディングは同じ11の適合性テストケースでテストされます。各テストは`.synx`入力ファイルと`.expected.json`ファイルで構成されます。11テストすべてで同一のJSONを生成するバインディングが適合とみなされます。

| # | 名前 | テスト内容 |
|---|------|---------------|
| 01 | `scalar-types` | すべてのスカラー：string、int、float、bool、null |
| 02 | `nesting` | ネストされたオブジェクト（3+レベル） |
| 03 | `arrays` | スカラーとオブジェクトの配列 |
| 04 | `type-casting` | `key(int)`、`key(float)`、`key(bool)`、`key(string)` |
| 05 | `comments` | `#`、`//`、`### ... ###`複数行コメント |
| 06 | `multiline` | インデントによる複数行値 |
| 07 | `mixed` | 混合構造：同レベルのオブジェクト＋配列 |
| 08 | `strings-with-spaces` | クォートなしのスペース含む値 |
| 09 | `empty-values` | `key ""`（空文字列）、`key ~`（null） |
| 10 | `tool-mode` | `!tool`と`!schema` — 出力の再構造化 |
| 11 | `llm-directive` | `!llm` — データツリー変更なし |
...
SYNXは悪意のある入力から保護するためにハードリミットを適用します：
...
| :calc式の長さ | 4,096 文字 |
| :includeの深度 | 16 レベル |
| :includeのファイルサイズ | 10 MB |

**ファジング:**

パーサーは3つのターゲットで継続的にファズテストされています：

- `fuzz_parse` — パーサー＋エンジン（任意の入力）
- `fuzz_compile` — バイナリコーデックのラウンドトリップ（compile → decompile）
- `fuzz_format` — フォーマッターの安定性

ファズコーパスには、長時間のファジングセッションで発見された**7,177**の興味深い入力が含まれています。これらはCIの各実行時にリグレッションテストとして使用されます。
...
**入力検証：** サイズ制限なしで信頼できないSYNXをパースしないでください。パーサーはハードリミット（16 MiB、深度128）を適用しますが、アプリケーションレベルのチェックも追加すべきです。

**環境マーカー：** `:env`マーカーはプロセス環境から読み取ります。信頼できないユーザーがSYNXソースに影響を与えられるコンテキストで、機密環境変数がアクセスできないことを確認してください。

**Includeパス：** `:include`マーカーはドキュメントからの相対パスを解決します。信頼できない入力シナリオでは、`SYNX_DISABLE_INCLUDE=1`またはAPIフラグ`ParseOptions::no_includes()`で`:include`を無効にしてください。

> **信頼できない`!active`ドキュメントで`:secret`をパースしないでください。**`:secret`マーカーはシークレットバックエンドに接続します。`!active`ドキュメントは信頼できるソースからのみ処理してください。

### FAQ

**なぜYAMLではないのですか？**
YAMLには多くの落とし穴があります：ノルウェー問題（国コード`NO`が`false`になる）、自動型変換、マルチドキュメントストリーム、複雑なスコーピングルールのアンカー、SYNXのシンプルなモデルとは異なるホワイトスペースの扱い。SYNXはこれらのサプライズを排除するために機能面を意図的に絞っています。

**インデントにタブを使えますか？**
いいえ。タブはパースエラーです。2スペース（正規形）または任意の一貫したスペース数を使用してください。フォーマッターは2スペースに正規化します。

**スペースを含む文字列にクォートは必要ですか？**
いいえ。キー（およびオプションのマーカー）の後のすべてが値として扱われます（スペースを含む）。クォートは空文字列を表すためだけに必要です：`key ""`。

**常に`!active`が必要ですか？**
マーカー（`:env`、`:calc`など）や制約（`[type:int]`）が必要な場合のみです。動的解決のないデータファイルは静的モードで完璧に動作します。

**SYNXの出力は常に有効なJSONですか？**
はい。`synx parse`とすべての`parse()` APIはJSON互換の値を返します。`synx convert --to json`は厳密なJSONを生成します。

**仕様は変更される可能性がありますか？**
SYNX v3.6.0はフリーズされた仕様です。文法は変更されません。新機能（もしあれば）は追加的で、新しいメジャーバージョンでリリースされます。
