...
try {
    ...
    # --- Build manifest.json (VSIX v3: lists all payload files with SHA256) ---
    $totalSize = 0
    $fileEntries = [System.Collections.ArrayList]::new()
    foreach ($f in $files) {
        $fInfo = Get-Item $f.FilePath
        $hash  = Get-FileSha256 $f.FilePath
        [void]$fileEntries.Add([ordered]@{fileName = "/$($f.ZipPath)"; sha256 = $hash; size = [long]$fInfo.Length})
    ...
    $manifestObj = [ordered]@{
        ...
        installSizes = [ordered]@{ targetDrive = [long]$totalSize }
    }
    ...
    $catalogObj = [ordered]@{
        ...
        packages = @(
            [ordered]@{
                ...
                type = "Vsix"
                payloads = @(
                    [ordered]@{ fileName = $OutFile; size = 0 }
                )
                vsixId = $vsixId
    ...
    $catalogJson = $catalogObj | ConvertTo-Json -Depth 5 -Compress:$false
