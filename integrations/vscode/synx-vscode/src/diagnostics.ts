  ...
  for (const node of parsed.allNodes) {
    ...
    if (node.markers.length > 0) {
      ...
      if (node.markers.includes('alias') && node.rawValue && parsed.mode === 'active') {
        ...
        // Skip check when the alias target's root key is an :include/:import
        // — included file keys are not in keyMap but will be resolved at runtime
        const refRoot = ref.includes('.') ? ref.split('.')[0] : null;
        const sibRoot = siblingPath.includes('.') ? siblingPath.split('.')[0] : null;
        const isIncluded = (key: string | null) => key != null && parsed.keyMap.has(key) &&
          parsed.keyMap.get(key)!.markers.some(m => m === 'include' || m === 'import');
        const targetInInclude = isIncluded(refRoot) || isIncluded(sibRoot);
        if (!targetInInclude && !parsed.keyMap.has(ref) && !parsed.keyMap.has(siblingPath)) {
