...
namespace SynxLanguageService.Classification
{
    ...
    internal class SynxClassifier : IClassifier
    {
        ...
        private static readonly Regex InlineCommentRe = new(@"\s+(//|#)(.*)");
        ...
        private void ClassifyValue(List<ClassificationSpan> result, ITextSnapshot snapshot, int start, string value)
        {
            ...
            // Check for inline comments before fallback string
            var inlineComment = InlineCommentRe.Match(value);
            if (inlineComment.Success)
            {
                int valLen = inlineComment.Index;
                if (valLen > 0)
                    result.Add(MakeSpan(snapshot, start, valLen, "synx.value.string"));
                result.Add(MakeSpan(snapshot, start + inlineComment.Index, inlineComment.Length, "synx.comment"));
                return;
            }
